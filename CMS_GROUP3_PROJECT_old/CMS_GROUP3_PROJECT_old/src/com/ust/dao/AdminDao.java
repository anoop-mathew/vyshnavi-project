...
public class AdminDao implements AdminDaoInterface {
	...
	// verify doctor login
		...
		public AdminRole selectDocRole(String username, String password) {
			String sql =" select cd.dId from cm_doctorTable cd join cm_staffTable cs on cd.sId = cs.sId where username='"+username+"' and password='"+password+"'";
	...
	public List<Staff> getStaff() {
		return template
				.query("select sId,sName,roleName,sPhNo,sEmail,sGender,sAddr,sExp,isActive,createdDate,dob from cm_staffTable stf join cm_roleTable role on(stf.roleId=role.roleId) where isActive='0' and (role.roleName='Admin'or role.roleName='Reception')",
						new RowMapper<Staff>() {
		...
		public List<DoctorStaff> getDoctor() {
			return template
					.query("select stf.sName,stf.sGender,stf.sAddr,stf.sExp,stf.sPhNo,stf.sEmail,stf.isActive"
							...
							+ "cm_staffTable stf on(doc.sId=stf.sId) join cm_consultationdays cs on(doc.dId=cs.dId) where stf.isActive='0'",
	...
	// view doctor by id
		...
		public DoctorStaff getDocById(int dId) {
			...
			String sql="select stf.sName,stf.sGender,stf.sAddr,stf.sExp,stf.sPhNo,stf.sEmail,stf.isActive"
					...
					+ "cm_staffTable stf on(doc.sId=stf.sId) join cm_consultationdays cs on(doc.dId=cs.dId) where doc.did="+dId;
			return template.queryForObject(sql, new Object[] {},
	...
	public int updateMed(Medicine a1,int medId) {
		...
		+ "',medPrice=" + a1.getMedPrice() + ",isActive=" + 0
				...
				+ "',createdDate= TO_DATE('"
						+ java.time.LocalDate.now() + "','YYYY-MM-DD')  where medId="
				+ medId;
	...
	public int saveStaff(Staff staff, String roleName) {
			...
			+ "("+ ad.getRoleId()+",'"+staff.getsName()+"',"
			+"TO_DATE('"+sqlDate+"','YYYY-MM-dd')"
			+",'" +staff.getsGender()+"','"
			+staff.getsAddr()+"','"+staff.getsExp()+"','"
			+staff.getsPhNo()+"','"+staff.getsEmail()+"','"
	...
	//save patient
		...
		public int saveDoctor(DoctorStaff ds, String roleName) {
			...
			String sql2 = "insert into cm_consultationDays(dId,sunday,monday,tuesday,wednesday,thursday,friday,saturday) values ("+ maxDid+ ",'"+ ds.getSunday() + "','"+ ds.getMonday() + "','"+ ds.getTuesday() + "','"+ ds.getWednesday() + "','"+ ds.getThursday() + "','"+ ds.getFriday() + "','"+ ds.getSaturday() + "')"; 
  ...
  // get available doctors
	...
	public List<Staff> getAvailableDoctorByDay() {
		...
		return template.query("select cm_stafftable.sName,cm_doctorTable.dSpec,cm_doctorTable.dId from cm_staffTable "
				+ "join cm_doctorTable on(cm_staffTable.sId=cm_doctorTable.sId) "
				+ "join cm_consultationDays on(cm_doctorTable.dId=cm_consultationDays.dId)" + " where " + day + "='1'",
				new RowMapper<Staff>() {
		...
		public List<DoctorAppointment> getTodaysDoctorAppointment(int dId) {
			...
			return template
					.query("select ca.appId,ca.regId,cp.pFName,cp.pLName,consultStatus from cm_appoinmentTable ca"
							...
							+ dId+" and dateofApp='"+formatter.format(dt).toUpperCase()+"'",
		...
		public List<DoctorAppointment> getTodaysAppointment() {
			...
			return template
					.query("select ca.appId,ca.regId,cp.pFName,cp.pLName,cs.sName, consultStatus from cm_appoinmentTable ca"
							+ " join cm_patientTable cp on ca.regId=cp.regId join cm_doctorTable cd on "
							+ "ca.dId=cd.dId join cm_staffTable cs on cd.sid=cs.sid where ca.dateOfApp='"+formatter.format(dt).toUpperCase()+"'",
							new RowMapper<DoctorAppointment>() {
						public DoctorAppointment mapRow(ResultSet rs, int row)
								throws SQLException {
							DoctorAppointment dj = new DoctorAppointment();
							dj.setAppId(rs.getInt(1));
							dj.setRegId(rs.getInt(2));
	...
	public long getDocId(String date,int regId){
		...
		String sql="select did from cm_doctorobstable where regId="+regId+ " AND obserDate='"+date+"'";
	...
	public int addPrescription(Prescription prescription,String date){
		...
		sql="insert into cm_prescriptiontable (dId,regId) values("+docId+","+prescription.getRegId()+")";
	...
	public int addPrescDetail(List<PrescDetail>prescDetail){
		...
		for(PrescDetail presc:prescDetail){
			sql="insert into cm_prescDetailTable(prescId,medId,medFreq,medDays,prescDate,pharmacyStatus) values("+presc.getPrescId()+","
					+presc.getMedId()+",'"+presc.getMedFreq()+"',"+presc.getMedDays()+",TO_DATE('"+sqlDate+"','YYYY-MM-dd') ,'"+presc.getPharmacyStatus()+"')";
	...
	// to display prescriptionDetail
		...
		public int updatePatient(Patient pa)  {
			...
			String sql = "update cm_patientTable set pFName='"
					+ pa.getpFName()
					+ "',pLName ='"
					+ pa.getpLName()
					+ "',pGender ='"
...
//			Integer maxId = getSequence();
			...
			String sql1 = "update cm_doctorTable set sId="
					+ sId
					+ ",dSpec='"
					+ doc.getdSpec()
					+ "',dQualification='"
					+ doc.getdQualification()
					+ "',consultFee=" + doc.getConsultFee()+ "where dId = "+doc.getdId();

			template.update(sql1);
			
//			Integer maxDid =getDocSequence();
			int cId=getConsltationId(doc.getdId());
			String sql2 = "update cm_consultationDays set dId="+ doc.getdId()+ ",sunday='"+ doc.getSunday() + "',monday='"+ doc.getMonday() + "',tuesday='"+ doc.getTuesday() + "',wednesday='"+ doc.getWednesday() + "',thursday='"+ doc.getThursday() + "',friday='"+ doc.getFriday() + "',saturday='"+ doc.getSaturday() + "' where consultDayId = "+cId; 
		...
		private int getConsltationId(int dId) {
		    ...
		    String sql="select consultDayId from cm_consultationDays where dId="+dId;
		...
		//update staff
				...
				public int updateStaff(Staff st,String roleName )  {
					...
					AdminRole ad = getRoleName(roleName);
					System.out.println("roleId="+ad.getRoleId());
					String sql="update cm_staffTable set roleId="+ ad.getRoleId()+",sName='"+st.getsName()+"',DOB="
							+"TO_DATE('"+sqlDate+"','YYYY-MM-dd')"
							+",sGender='" +st.getsGender()+"',sAddr='"
							+st.getsAddr()+"',sExp='"+st.getsExp()+"',sPhNo='"
							+st.getsPhNo()+"',sEmail='"+st.getsEmail()+"',username='"
							+st.getUsername()+"',password='"+st.getPassword()+"',isActive='"
							+st.getIsActive()+"',createdDate="
							+"TO_DATE('"+java.time.LocalDate.now()+"','YYYY-MM-DD')"
							+ "where sId ="+st.getsId();
					return template.update(sql);
				...
				return template.query("SELECT cm_LabTestTable.lName,cm_LabTestTable.labId,To_char(cm_assignlabTable.assiglabDate,'yyyy-mm-dd') "
				+ " from cm_labTestTable join cm_assignlabTable on cm_labTestTable.labId=cm_assignlabTable.labId where cm_assignlabTable.regId= "+ regId + " order by cm_assignlabTable.assiglabDate", new RowMapper<PatHistory>() {
				...
				return template.query("SELECT cm_prescdetailTable.prescId,cm_medicineTable.medName,To_char(cm_prescdetailTable.prescdate,'yyyy-mm-dd')"
				+ " from cm_medicineTable join cm_prescdetailTable on cm_medicineTable.medId=cm_prescdetailTable.medId join cm_prescriptionTable on cm_prescdetailTable.prescId=cm_prescriptionTable.prescId where cm_prescriptionTable.regId= "+ regId + " order by cm_prescdetailTable.prescdate ", new RowMapper<PatHistory>() {
				public PatHistory mapRow(ResultSet rs, int row) throws SQLException {
				PatHistory patHistory = new PatHistory();
				...
				return template.query("SELECT cm_staffTable.sName,cm_doctorObsTable.obsnotes,To_char(cm_doctorObsTable.obserdate,'yyyy-mm-dd')"
				+ " from cm_staffTable join cm_doctorTable on cm_staffTable.sId=cm_doctorTable.sId join cm_doctorObsTable on cm_doctorTable.dId=cm_doctorObsTable.dId where cm_doctorObsTable.regId= "+ regId + " order by cm_doctorObsTable.obserdate ", new RowMapper<PatHistory>() {
				public PatHistory mapRow(ResultSet rs, int row) throws SQLException {
				PatHistory patHistory = new PatHistory();
